...
const FEATURES: [string, string, string][] = [
  ...
  [
    "✦",
    "#4da8ff",
    "Real drafts, not suggestions — from a Writer who actually writes",
  ],
  [
    "◎",
    "#c89cff",
    "Director synthesizes every exchange and names the next move",
  ],
...
export default function LoginPage() {
  ...
  const handleSignIn = (provider: string) => {
    setLoading(provider);
    trackSignInStarted({ auth_provider: provider, entry_point: "login_page" });
    ...
    >
      ...
      >
        ...
        <div>
          ...
          >
            <div style={{ display: "flex", gap: 6 }}>
              {AGENTS.map((a) => (
                <span
                  key={a.id}
                  style={{ color: a.color, fontSize: 16, lineHeight: 1 }}
                >
                  {a.icon}
                </span>
              ))}
          ...
          >
            A Researcher to ground you. A Writer who actually drafts. An Editor
            who tightens. A Critic who pushes back. A Director who synthesizes
          ...
          <div
            style={{
              padding: "14px 16px",
              background: "#c89cff0f",
              border: "1px solid #c89cff44",
              borderLeft: "2px solid #c89cff",
          ...
          >
            ...
            >
              "The Researcher grounds us in what's verified. The Writer drafts a
              real version. The Critic finds the three weakest points. Taking it
              all together — here's the next move."
            </p>
          </div>
        </div>
        ...
        >
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 9 }}>
            ...
            >
              {loading === "google" ? (
                <span
                  style={{ fontFamily: T.mono, fontSize: 11, color: "#666" }}
                >
                  Redirecting…
                </span>
              ) : (
            ...
            <button
              ...
              onMouseEnter={(e) => {
                if (!loading) e.currentTarget.style.background = "#2d333b";
              }}
              onMouseLeave={(e) => {
                if (!loading) e.currentTarget.style.background = "#24292e";
              }}
            >
              {loading === "github" ? (
                <span
                  style={{ fontFamily: T.mono, fontSize: 11, color: "#aaa" }}
                >
                  Redirecting…
                </span>
              ) : (
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {FEATURES.map(([icon, color, text]) => (
              ...
              >
                <span style={{ color, fontSize: 13, width: 14, flexShrink: 0 }}>
                  {icon}
                </span>
                {text}
              </div>
